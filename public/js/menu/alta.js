class AltaForm {
  ...
  constructor(renderTablaAlta, saveProduct) {
    ...
    this.inputs = document.querySelectorAll(
      ".alta-container form input.data-validation"
    );
      ...
      if (input.type != "checkbox") {
        input.addEventListener("input", () => {
          ...
          if (renderTablaAlta)
            renderTablaAlta(
              !this.invalidFields(),
              productoController.productsList
            );
        });
      }
    });
    ...
    this.form.addEventListener("submit", (e) => {
      //porque poner este y el anterior recorrido para renderizar la tabla dentro del constructor?
      e.preventDefault();
...
async function initAlta() {
  ...
  formularioAlta = new AltaForm(
    renderTablaAlta,
    productoController.saveProduct
  );
